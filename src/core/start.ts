...
import { getAgentAdapter, type AgentCapabilities, type AgentType, type FormattedAgentRequest } from "../adapters/agent-runner.js";
...
export type NaviStartResult = {
  readonly state: NaviState;
  ...
  readonly agent: {
    ...
    readonly validConfig: boolean;
    readonly request: FormattedAgentRequest;
  };
};

export async function runNaviStart(projectDir: string, agentType: AgentType | string = "gemini"): Promise<NaviStartResult> {
...
export function startReport(result: NaviStartResult): string {
  return [
...
export function startJson(result: NaviStartResult): string {
