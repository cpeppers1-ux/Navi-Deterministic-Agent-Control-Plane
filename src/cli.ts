...
function printMissingState(): void {
...
async function main(): Promise<void> {
  ...
  if (command === "run" || command === "execute") {
    ...
    if (!result.authorized || !result.execution.passed) process.exitCode = 1;
    return;
  }
    ...
    if (subcommand === "list") {
      ...
      if (jsonOutput) {
        ...
        if (checkpoints.length === 0) {
          ...
          for (const cp of checkpoints) console.log(checkpointReport(cp));
        }
      }
      return;
    }
    const cp = await loadLatestCheckpoint(projectDir);
    if (!cp) { console.log("No checkpoint found."); return; }
